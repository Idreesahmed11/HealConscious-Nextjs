...
function StarIcon({ filled }: { filled: boolean }) {
  return (
    <svg
      className={`w-4 h-4 ${filled ? 'text-gray-400' : 'text-gray-300'}`}
      fill="currentColor"
      viewBox="0 0 20 20"
    >
...
function GooglePayModal({ onClose }: { onClose: () => void }) {
  const modalRef = useRef<HTMLDivElement>(null);
  useOutsideClick(modalRef, onClose);
  useLockBody();
  ...
  const countries = ['Pakistan', 'United States', 'United Kingdom', 'India', 'Australia', 'Canada', 'Germany', 'France', 'UAE'];

  const inputCls = 'w-full border border-gray-300 rounded px-3 py-3 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-gray-800 placeholder-gray-400 transition-all';
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 px-5 py-5">
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <button className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-gray-500 text-xs font-bold flex-shrink-0 hover:border-gray-500 transition-colors">?</button>
          </div>

          
          <div className="flex gap-3 mb-4">
            <input type="text" placeholder="MM/YY" value={expiry} onChange={(e) => setExpiry(formatExpiry(e.target.value))} className={`${inputCls} flex-1`} inputMode="numeric" />
            <div className="flex items-center gap-2 flex-1">
              <input type="text" placeholder="Security code" value={cvv} onChange={(e) => setCvv(e.target.value.replace(/\D/g, '').slice(0, 4))} className={`${inputCls} flex-1 min-w-0`} inputMode="numeric" />
          ...
          <div className="relative mb-4">
            <input
              ...
              className="w-full border border-gray-300 rounded px-3 pt-5 pb-2 text-sm outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-gray-800 transition-all"
              placeholder=" "
            />
          ...
          <input type="text" placeholder="Street address" value={street} onChange={(e) => setStreet(e.target.value)} className={`${inputCls} mb-4`} />
          <input type="text" placeholder="Apt, suite, etc. (optional)" value={apt} onChange={(e) => setApt(e.target.value)} className={`${inputCls} mb-4`} />
          <input type="text" placeholder="Suburb" value={suburb} onChange={(e) => setSuburb(e.target.value)} className={`${inputCls} mb-4`} />
          <input type="text" placeholder="City" value={city} onChange={(e) => setCity(e.target.value)} className={`${inputCls} mb-4`} />
          <input type="text" placeholder="Postal code" value={postal} onChange={(e) => setPostal(e.target.value)} className={`${inputCls} mb-6`} inputMode="numeric" />
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
      ...
      >
        ...
        <div className="px-8 py-8 flex flex-col items-center">
          ...
          <h3 className="text-2xl font-bold text-gray-900 mb-1 text-center">Fast, secure checkout</h3>
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-14">
          ...
          <div className="w-full lg:flex-1 flex flex-col">
            ...
            <p className="text-sm text-gray-500 mb-1">
              Read our{' '}
              <button onClick={() => setShowShipping(true)} className="underline text-gray-700 hover:text-gray-900 transition-colors">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Read our{' '}
              <button onClick={() => setShowRefund(true)} className="underline text-gray-700 hover:text-gray-900 transition-colors">
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <button
                ...
                className={`flex-1 h-11 rounded-lg text-sm font-semibold tracking-wider transition-all duration-300 text-white ${added ? 'bg-green-600' : 'bg-gray-900 hover:bg-gray-700'}`}
              >
            ...
            >
              <span className={wishlist ? 'text-red-500' : 'text-gray-400'}>
          ...
          <div className="py-10 text-gray-600 leading-relaxed text-[15px]">
            ...
            {activeTab === 'description' && (
              <div className="space-y-5 max-w-3xl">
                ...
                <h6 className="font-semibold text-gray-900">Product Description</h6>
                <p>
                  Careox is a 28-day mental healthcare guide designed to help you release buried pain, ease daily stress, and break free from social pressures. Curated by psychologists and expert coaches, it blends reflection, healing practices, and practical exercises. The guide helps you reconnect with your true self, access the subconscious mind, and move forward with clarity and strength.
                </p>
                <div>
            ...
            {activeTab === 'reviews' && (
              <div className="max-w-2xl">
                ...
                <h6 className="font-semibold text-gray-900 mb-6">
                  Be the first to review &quot;Careox | 4 week mental health fortification program&quot;
                </h6>
                <div className="border border-gray-200 rounded-xl p-6 md:p-8">
                  ...
                  <div className="space-y-4">
                    ...
                    <textarea
                      ...
                      className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm h-32 outline-none focus:border-gray-400 resize-none transition-colors"
                    />
